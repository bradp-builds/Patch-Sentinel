...
def process_zip_data(config, zip_bytes, db_adapter, send_notify_func):
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), 'r') as outer_zip:
        ...
        zip_obj = zipfile.ZipFile(io.BytesIO(outer_zip.read(inner_zips[0])), 'r') if inner_zips else outer_zip
        
        try:
            delta_paths = [name for name in zip_obj.namelist() if name.endswith('delta.json')]
            target_cves = set()
            ...
            cve_path_map = {os.path.splitext(os.path.basename(n))[0]: n for n in zip_obj.namelist() if n.endswith('.json')}
            ...
            for cve_id in sorted(target_cves):
                ...
                try:
                    ...
                    for node in affected_nodes:
                        ...
                        if original_name and any(fnmatch.fnmatch(original_name.lower(), pat.lower()) for pat in monitored_sources):
                            matched = True
                    ...
                    if matched:
                        ...
                        if db_adapter.has_notified_today(cve_id, local_date_str):
                            print(f"ℹ️ {cve_id} already updated today ({local_date_str}). Suppressing duplicate spam.")
                            continue
                            
                        send_notify_func(config, cve_id, matched_product, severity, desc_text)
...
def run_engine(config, db_adapter, fetch_func, send_notify_func):
    ...
    unprocessed = [(dt, rid) for dt, rid in lookback_releases if not db_adapter.is_diff_processed(rid)]
            
    is_test = config.get("test_mode", False)
    if not is_test and len(unprocessed) > 3:
        print(f"⚠️ Caught {len(unprocessed)} unprocessed hours. Enforcing safety execution cap of 3.")
    ...
    for dt, release_id in unprocessed:
        ...
        if zip_bytes is None:
            print(f"⚠️ Release data {release_id} not available yet (404). Will retry next run.")
            continue
