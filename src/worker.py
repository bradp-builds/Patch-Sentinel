...
class CloudflareDBAdapter(cve_engine.DBAdapter):
    ...
    def is_diff_processed(self, release_id: str) -> bool:
        return self.db.prepare("SELECT 1 FROM processed_diffs WHERE release_id = ?").bind(release_id).first() is not None

    def mark_diff_processed(self, release_id: str):
        self.db.prepare("INSERT OR IGNORE INTO processed_diffs (release_id) VALUES (?)").bind(release_id).run()

    def has_notified_today(self, cve_id: str, local_date: str) -> bool:
        return self.db.prepare("SELECT 1 FROM sent_notifications WHERE cve_id = ? AND local_date = ?").bind(cve_id, local_date).first() is not None

    def record_notification(self, cve_id: str, local_date: str):
        self.db.prepare("INSERT OR IGNORE INTO sent_notifications (cve_id, local_date) VALUES (?, ?)").bind(cve_id, local_date).run()

def cf_fetch(url):
    async def _async_fetch():
        res = await fetch(url)
        if res.status == 404: return None
        if res.status != 200: raise Exception(f"GitHub release returned status {res.status}")
        buf = await res.arrayBuffer()
        return bytes(buf.to_py())
    return asyncio.run(_async_fetch())

def cf_notify(config, cve_id, product, severity, desc):
    if config.get("test_mode", False): return
    ...
    async def _async_send():
        hdrs = Headers.new({"Content-Type": "application/json", "User-Agent": "PatchSentinel/1.0"})
...
def _build_config(env):
    ...
    config = {
        ...
        "providers": {provider: {"webhook_url": env.DISCORD_WEBHOOK_URL if provider == "discord" else env.SLACK_WEBHOOK_URL}},
        "test_mode": str(getattr(env, "TEST_MODE", "")).lower() == "true",
        "timezone": getattr(env, "TIMEZONE", "America/Detroit"),
        "min_severity_score": float(env.MIN_SEVERITY_SCORE) if getattr(env, "MIN_SEVERITY_SCORE", None) else None,
        "monitored_sources": []
    }
